...
export const isCreateGroupCustomMessage = (message: IMessageModel) => {
  if(message.type === TUIChatEngine.TYPES.MSG_CUSTOM){
    ...
    (
      ...
      message?.payload?.description === "group_delete" || 
      message?.payload?.description ===  "add_pinned_msgs" || 
      message?.payload?.description ===  "remove_pinned_msgs" || 
      message?.payload?.description ===  "GM_ORDER" || 
      message?.payload?.description ===  "c2c_set_top" || 
      message?.payload?.description ===  "c2c_rm_top" || 
      message?.payload?.description ===  "delete_message"
    )
    {
      return true;
    }
