	...
	<div class="TUIChat" :class="[!isPC ? 'TUIChat-H5' : '']">
		<div class="TUIChat-main">
	      ...
	      groupNotification
			...
			@scrolltolower='_scrolltoupper'
			@scrolltoupper='_scrolltolower'
			:scroll-into-view='intoViewID'>
				...
				>
				  {{isGetAllData?$tc('暂无更多数据'):''}}
				</p>
				<!-- <view class="h80 flex-center font-22 less-color"
					>{{isGetAllData?$tc('暂无更多数据'):''}}</view> -->
				...
				<view v-for="(item, index) in messageList" :id="`zp-id-${item.ID}`" :key="item.ID"
					...
					<view >
						...
						<div class="message-item" @click.stop="messageClick" @touchstart="messageTouchstart(item)">
							...
							<div v-else-if="!item.isRevoked && !isPluginMessage(item)" :id="`msg-bubble-${item.ID}`"
								...
								<MessageBubble :messageItem="item" :blinkMessageIDList="blinkMessageIDList"
									...
									<MessageText v-if="item.type === TYPES.MSG_TEXT" :messageItem="{
					...
					progress:item.progress
									...
									<MessageVideo v-else-if="item.type === TYPES.MSG_VIDEO"
				...
				<view class="h100 flex-center font-22 less-color pd-lr-40"
					v-if="!isMyFriend">
...
<script lang="ts" setup>
	...
	const scrollToBottom = () => {
		...
		// scrollTop.value += 300;
		nextTick(()=>{
			intoViewID.value= `msg-bubble-${messageList.value[messageList.value.length-1].ID}` 
		})
		
		console.log('intoViewID.value',intoViewID.value)
	...
	function onMessageListUpdated(list : IMessageModel[]) {
		...
		list = list
			.filter((item) => {
				let { payload, conversationID, cloudCustomData, status, progress } = item
				// item.messageItem={
				// 	payload,
				// 	conversationID,
				// 	cloudCustomData,
		...
		if (messageList?.value && messageList?.value?.length != 0) {
			if (messageList?.value.length != list.length) { }
	...
	const cutMessageList = computed(() => {
		...
		return messageList.value.filter((item, index) => index < messageCout)
	})
	...
	function getHistoryMessageList() {
		...
		intoViewID.value=`msg-bubble-${messageList.value[0].ID}` 
		// })
